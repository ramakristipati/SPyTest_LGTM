...
def test_FtOpSoRoEvpnRouterLvtepFt32317(Tgencleanup_fixture):
    ...
    traffic_status = True
    for i in range(4):
        result=validate_tgen_traffic(traffic_details=traffic_details, mode="aggregate", comp_type="packet_rate",
                tolerance_factor=2)
        if result is False:
            st.wait(1,"waiting for 1 sec before retry traffic verification")
            continue
        else:
            break
...
def test_FtOpSoRoEvpnRouterLvtepFt32314(Lvtep32314_fixture):
    ...
    arp.delete_static_arp(evpn_dict["leaf_node_list"][0], '30.1.1.100',
                          "Vlan{}".format(evpn_dict['leaf1']["tenant_l3_vlan_list"][0]),vrf=evpn_dict["leaf1"]["vrf_name_list"][0])
    arp.delete_static_arp(evpn_dict["leaf_node_list"][1], '30.1.1.100',
                          "Vlan{}".format(evpn_dict['leaf1']["tenant_l3_vlan_list"][0]),vrf=evpn_dict["leaf2"]["vrf_name_list"][0])
    arp.delete_static_arp(evpn_dict["leaf_node_list"][0], '120.1.1.100',
                          "Vlan{}".format(evpn_dict["l3_vni_sag"]["l3_vni_sagvlan_list"][0]),vrf=evpn_dict["leaf1"]["vrf_name_list"][0])
...
def test_FtOpSoRoEvpnRouterLvtepFt32327(Tgencleanup_fixture):
    ...
    leaf1_uplink_list = [evpn_dict["leaf1"]["pch_intf_list"][0],evpn_dict["leaf1"]["intf_list_spine"][3]] \
                       +[evpn_dict["leaf1"]["pch_intf_list"][1],evpn_dict["leaf1"]["intf_list_spine"][7]]
    leaf2_uplink_list = [evpn_dict["leaf2"]["pch_intf_list"][0],evpn_dict["leaf2"]["intf_list_spine"][3]] \
                       +[evpn_dict["leaf2"]["pch_intf_list"][1],evpn_dict["leaf2"]["intf_list_spine"][7]]
    hdrMsg("\n####### Step 11: Verify BUM traffic from LVTEP to SVTEP before links are down  ##############\n")
    if int(float(leaf1_rx[0]['rx_bps'])) > 350:
        for interface1 in leaf1_uplink_list:
            tx = get_interfaces_counters(evpn_dict["leaf_node_list"][0],
                            interface=interface1,property="tx_bps",cli_type="click")
            ktx = Evpn.get_port_counters(evpn_dict["leaf_node_list"][0], interface1, "tx_bps")
            tx_val = ktx[0]['tx_bps'].split(" ")
            ...
            if verify_vxlan_traffic(ktx,tx_val,"encap"):
                st.log("INFO: Leaf 1 BUM forwader port is : {}".format(interface1))
                lvtep_bum_forwarder_port1.append(interface1)
            else:
                st.log("INFO: interface to be brought down is : {}".format(interface1))
                port.shutdown(evpn_dict["leaf_node_list"][0],[interface1])

        if len(lvtep_bum_forwarder_port1) > 0:
        ...
        for interface1 in leaf2_uplink_list:
    ...
    if int(float(leaf1_rx[0]['rx_bps'])) > 350:
        for interface1 in leaf1_uplink_list:
            ...
            if verify_vxlan_traffic(ktx,tx_val,"encap"):
                st.log("INFO: Leaf 1 BUM forwader port is : {}".format(interface1))
                lvtep_bum_forwarder_port2.append(interface1)
            else:
                st.log("INFO: Leaf 1 interface not carrying traffic is : {}".format(interface1))

        if len(lvtep_bum_forwarder_port2) > 0:
    ...
    result = False 
    for intf1 in lvtep_bum_forwarder_port1:
        for intf2 in lvtep_bum_forwarder_port2:
            if intf1 == intf2:
                result = True
                st.log("INFO: Same LVTEP BUM forwarder port {} was found across link flap".format(intf1))
