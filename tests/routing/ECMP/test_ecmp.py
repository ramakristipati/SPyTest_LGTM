...
def config_base_all():
    ...
    [res, _] = utils.exec_all(True, [[config_base_dut1], [config_base_dut2]])
...
def verify_config_base_all():
    ...
    def verify_config_base_dut2():
        ...
        return res[0] if len(res)==1 else False
    [res, _] = utils.exec_all(True, [[verify_config_base_dut1], [verify_config_base_dut2]])
...
def deconfig_base_all():
    ...
    [res, _] = utils.exec_all(True, [[deconfig_base_dut1], [deconfig_base_dut2]])
    return False if False in set(res) else True
...
def config_base_tg():
    ...
    tg_tr12_6=tg.tg_traffic_config(port_handle=tg_1, mode='create', transmit_mode='continuous', length_mode='fixed', rate_pps=data.tg_rate, l3_protocol='ipv6', mac_src=data.tg_macs[0], mac_discovery_gw=data.ip6_1[0], mac_dst=data.dut1_mac, ipv6_src_addr=data.ip6_1[1], ipv6_dst_addr=data.st_ip6_2[2], ipv6_dst_count=data.tg_count, ipv6_dst_mode='increment', ipv6_dst_step=data.tg_step6, frame_size=data.tg_framesize)
    tg_tr21_6=tg.tg_traffic_config(port_handle=tg_2, mode='create', transmit_mode='continuous', length_mode='fixed', rate_pps=data.tg_rate, l3_protocol='ipv6', mac_src=data.tg_macs[1], mac_discovery_gw=data.ip6_2[0], mac_dst=data.dut2_mac, ipv6_src_addr=data.ip6_2[1], ipv6_dst_addr=data.st_ip6_1[2], l4_protocol='tcp', tcp_src_port=data.src_port, tcp_dst_port=data.dst_port, tcp_dst_port_count=data.tg_count, tcp_dst_port_step=1, tcp_dst_port_mode='increment', frame_size=data.tg_framesize)
...
def prologue_epilogue():
    ...
    initialize_topology()
    [res, _] = utils.exec_all(True, [[config_base_tg], [config_base_all]], True)
    if res[1] is False:
        more_debugs(duts=dut_list)
        st.report_fail("module_config_verification_failed")
    st.banner("End of PROLOGUE.")
    yield
    [res, _] = utils.exec_all(True, [[deconfig_base_tg], [deconfig_base_all]], True)
...
def test_ecmp_cli001():
    ...
    def c1_t1_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t1_1], [c1_t1_2]])
    if False in set(res):
    ...
    def c1_t1a_2():
        ...
        if res1 is False or res2 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t1a_1], [c1_t1a_2]])
    if False in set(res):
    ...
    def c1_t1b_2():
        ...
        if res1 is False or res2 is True:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t1b_1], [c1_t1b_2]])
    if False in set(res):
    ...
    def c1_t2_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t2_1], [c1_t2_2]])
    if False in set(res):
    ...
    def c1_t3_2():
        ...
        if res1 is False or res2 is True:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t3_1], [c1_t3_2]])
    if False in set(res):
    ...
    def c1_t4_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t4_1], [c1_t4_2]])
    if False in set(res):
    ...
    def c1_t6_2():
        ...
        if res1 is False or res2 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t6_1], [c1_t6_2]])
    if False in set(res):
    ...
    def c1_t7_2():
        ...
        if res1 is False or res2 is True:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t7_1], [c1_t7_2]])
    if False in set(res):
    ...
    def c1_t8_2():
        ...
        if res1 is False or res2 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t8_1], [c1_t8_2]])
    if False in set(res):
    ...
    def c1_t12_2():
        ...
        if res1 is False or res2 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[c1_t12_1], [c1_t12_2]])
    if False in set(res):
...
def test_ecmp_basic_func001():
    ...
    def f1_t1_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[f1_t1_1], [f1_t1_2]])
    if False in set(res):
        fail_msg = "ERROR: Step T1 show cli for default values failed."
        fail_msgs += fail_msg
        st.log(fail_msg)
    ...
    [res, _] = utils.exec_all(True, [[f1_f1_1, 1], [f1_f1_2, 1]])
    tg.tg_traffic_control(action='clear_stats', port_handle=tg_all)
    tg.tg_traffic_control(action='run', handle=tg_strs)
    st.wait(waitvar)
    tg.tg_traffic_control(action='stop', handle=tg_strs)
    st.wait(waitvar/2)
    ...
    def f1_t4_2():
        ...
        if res1 is False or res2 is True:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[f1_t4_1], [f1_t4_2]])
    if False in set(res):
    ...
    [res, _] = utils.exec_all(True, [[f1_f1_1, 1], [f1_f1_2, 1]])
    tg.tg_traffic_control(action='clear_stats', port_handle=tg_all)
    tg.tg_traffic_control(action='run', handle=tg_v4s)
    st.wait(waitvar)
    tg.tg_traffic_control(action='stop', handle=tg_v4s)
    st.wait(waitvar/2)
    ...
    [res, _] = utils.exec_all(True, [[f1_f1_1, 1], [f1_f1_2, 1]])
    tg.tg_traffic_control(action='clear_stats', port_handle=tg_all)
    tg.tg_traffic_control(action='run', handle=tg_v6s)
    st.wait(waitvar)
    tg.tg_traffic_control(action='stop', handle=tg_v6s)
    st.wait(waitvar/2)
    res1=verify_intf_counters(rx=[[dut1, vars.D1T1P1], [dut2, vars.D2D1P1], [dut2, vars.D2D1P2]], ratio = [[1, 0, 1]], clear_save=True)
    res2=verify_intf_counters(rx=[[dut1, vars.D1T1P1]], tx=[[dut1, vars.D1D2P1], [dut1, vars.D1D2P2], [dut2, vars.D2T1P1]], ratio = [[1], [1, 0, 1]], saved_flag=True)
    if (res1 is False and res2 is False) or (res1 is True and res2 is True):
    ...
    def f1_t7_2():
        ...
        if res1 is True or res2 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[f1_t7_1], [f1_t7_2]])
    if False in set(res):
    ...
    def f1_t16_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[f1_t16_1], [f1_t16_2]])
    if False in set(res):
...
def test_ecmp_clos_func002():
    ...
    def f2_t2_2():
        ...
        return res[0] if len(res)==1 else False
    [res, _] = utils.exec_all(True, [[f2_t2_1], [f2_t2_2]])
    if False in set(res):
    ...
    def f2_t3_2():
        ...
        if res1 is False:
            ...
            return False
        return True
    [res, _] = utils.exec_all(True, [[f2_t3_1], [f2_t3_2]])
    if False in set(res):
    ...
    '''
    tg.tg_traffic_control(action='run', handle=[tg_v4s[0]])
    st.wait(waitvar)
    tg.tg_traffic_control(action='stop', handle=[tg_v4s[0]])
    st.wait(waitvar/2)
