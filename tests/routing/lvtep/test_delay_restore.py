...
def test_underlay(request):
    ...
    if carrier == "standby_node":
        ...
        evpn_dict["l2_traffic_loss_ccep"] = evpn_dict["l2_traffic_loss_ccep"] + 450000
    ...
    if carrier == "standby_node":
        ...
        evpn_dict["bum_traffic_loss_ccep"] = evpn_dict["bum_traffic_loss_ccep"] + 450000
    ############################################################################################
    ...
    if any(x in leaf1_rx[0]['rx_bps'] for x in rate):
        st.log("D3 receives traffic from mclag client")
        config_save_and_reboot(evpn_dict["leaf_node_list"][0],"normal")
        hdrMsg("STEP: Check the system status after normal reboot")
        if not basic_obj.poll_for_system_status(evpn_dict["leaf_node_list"][0]):
            st.error("LVTEP node 1 is not in ready state")
        res = False
        for i in range(1,50):
    ...
    if carrier == "standby_node":
        ...
        evpn_dict["l3_traffic_loss_ccep_reload"] = evpn_dict["l3_traffic_loss_ccep_reload"] + 450000
