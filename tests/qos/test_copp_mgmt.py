...
def test_ft_coppmgmt_burst_lldp():
    ...
    hdrMsg("STEP 1: Changing the queue shaper value to support higher CoPP rate limit 550 burst rate and 650 Burst rate for queues 10 and 11 ")
    shaper_data = {'policy_name': "copp-scheduler-policy", 'shaper_data': [{'queue': "10", 'pir': "73750", 'meter_type': 'bytes'}, \
                   {'queue': "11", 'pir': "86250", 'meter_type': 'bytes'}]}
    if not shaper.apply_queue_shaping_config(vars.D1, shaper_data):
        st.log("Config of the queue shaper value to support higher CoPP rate limit 550 burst rate and 650 burst rate for queues 10 and 11 failed")
        success = False
    ...
    if not copp_obj.verify_qos_scheduler_policy(dut=vars.D1,queue="10",sch_policy_name="copp-scheduler-policy",pir="590"):
        st.log("Change of the queue shaper value to support higher CoPP rate limit 590 burst rate for queues 10 FAIL")
        success = False
    if not copp_obj.verify_qos_scheduler_policy(dut=vars.D1,queue="11",sch_policy_name="copp-scheduler-policy",pir="690"):
        st.log("Change of the queue shaper value to support higher CoPP rate limit 690 Burst rate for queues 11 FAIL")
        success = False
    ...
    copp_obj.config_copp_action_group(vars.D1,copp_action_group="copp-user-lldp-action",trap_queue="10",cir="550",cbs="550",trap_action="trap")
    copp_obj.config_copp_action_group(vars.D1,copp_action_group="copp-user-lldp-action1",trap_queue="11",cir="650",cbs="650",trap_action="trap")
    ...
    res = copp_obj.verify_policy_type_copp(dut=vars.D1,copp_fgroup="copp-user-lldp",copp_agroup="copp-user-lldp-action", \
        cir="550",cbs="550",trap_queue="10",trap_action="trap")
    if not res:
        st.log("STEP 5: FAIL User defined CIR 550, CBS 650 burst rate do not show up in show policy type copp O/P")
        success = False
    ...
    tg_stream_handle = tg.tg_traffic_config(port_handle=tg_ph_1, mac_src="00:11:97:2F:8E:82", mac_dst="01:80:C2:00:00:0E",
                         mode='create', transmit_mode='multi_burst', l2_encap='ethernet_ii',burst_loop_count="35",pkts_per_burst="20",
                         data_pattern='02 07 04 00 11 97 2F 8E 80 04 07 03 00 11 97 2F 8E 82 06 02 00 78 00 00 00 00 '
                                      '00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00',
    ...
    hdrMsg("STEP 15: Verify LLDP burst bytes count should increase as CBS rate limit is increased from 550 PPS to 650 PPS")
    if drop2 > drop1:
    ...
    tolerance = (590*10)/100
    if drop2 < (590 + tolerance) and drop2 > (590 - tolerance):
    ...
    copp_obj.config_copp_action_group(vars.D1,copp_action_group="copp-user-lldp-action",trap_queue="10",cir="550",cbs="550",config="no")
    copp_obj.config_copp_action_group(vars.D1,copp_action_group="copp-user-lldp-action1",trap_queue="11",cir="650",cbs="650",config="no")
