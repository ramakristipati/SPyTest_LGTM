...
def L3ScaleEnhancement_Prologue_Epilogue(request):
    ...
    if route_max_flag:
        st.log("route_max_flag is {}. Configuring it accordingly.".format(route_max_flag))
        st.exec_all([[ipfeature.config_system_max_routes, dut1], [ipfeature.config_system_max_routes, dut2]])
        st.exec_all([[reboot_api.config_save, dut1], [reboot_api.config_save, dut2]])
        st.exec_all([[st.reboot, dut1], [st.reboot, dut2]])
...
def L3Scl_fixture_002(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    tg2.tg_emulation_bgp_control(handle=bgp_conf2['handle'], mode='stop')
...
def L3Scl_fixture_004(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    #
    tg1.tg_emulation_bgp_control(handle=bgp_rtr1['conf']['handle'], mode='stop')
    tg2.tg_emulation_bgp_control(handle=bgp_rtr2['conf']['handle'], mode='stop')
    tg1.tg_emulation_bgp_control(handle=bgp_conf['handle'], mode='stop')
    tg2.tg_emulation_bgp_control(handle=bgp_conf2['handle'], mode='stop')
    st.wait(7)
...
def L3Scl_fixture_005(request,L3ScaleEnhancement_Prologue_Epilogue):
    ...
    ############################################################################################
    mac_obj.get_mac_count(vars.D1)
    mac_obj.get_mac_count(vars.D2)
...
def test_L3Scl_002(L3Scl_fixture_002):
    ...
    (tg1, tg2, _, _, tg_ph_1, tg_ph_2, _, _) = get_handles()
    tg1.tg_traffic_control(action='reset',port_handle=tg_ph_1)
    tg2.tg_traffic_control(action='reset',port_handle=tg_ph_2)
    h3 = {}
    h4 = {}
...
def test_L3Scl_005(L3Scl_fixture_005):
    ...
    (_, _, tg3, _, _, _, tg_ph_3, _) = get_handles()
    ...
    for i in range(0,num_loops):
        ...
        tg3.tg_traffic_control(action='stop', handle=tr)
    ############################################################################################
...
def test_L3Scl_006(L3Scl_fixture_006):
    ...
    (tg1, _, tg3, _, tg_ph_1, _, tg_ph_3, _) = get_handles()
    ...
    h2 = tg3.tg_interface_config(port_handle=tg_ph_3, mode='config', ipv6_intf_addr=data.t1d1_ipv6_addr2,  ipv6_gateway=data.d1t1_ipv6_addr2, src_mac_addr='00:0b:01:00:00:01', vlan='1', vlan_id=data.vlan201, count=nd_cnt, arp_send_req='1', ipv6_gateway_step='::', ipv6_intf_addr_step='::1',ipv6_prefix_length = '64')

    tg1.tg_arp_control(handle=h1['handle'], arp_target='all')
    tg3.tg_arp_control(handle=h2['handle'], arp_target='all')
...
def test_L3Scl_ECMP_007(L3Scl_fixture_007):
    ...
    ############################################################################################
    for _,vlan,ip1,ip2 in zip(range(0,data.max_ecmp),data.dut1_vlan_scl,data.dut1_ecmp_ip,data.dut2_ecmp_ip):
    ...
    for _,ip1,ip2 in zip(range(0,data.max_ecmp+2),data.dut1_ecmp_ip,data.dut2_ecmp_ip):
...
def test_L3Scl_ECMP_008(L3Scl_fixture_008):
    ...
    ############################################################################################
    for _,vlan,ipv6_1,ipv6_2 in zip(range(0,data.max_ecmp),data.dut1_vlan_scl,data.dut1_ecmp_ipv6,data.dut2_ecmp_ipv6):
    ...
    for _,ip1,ip2 in zip(range(0,data.max_ecmp+2),data.dut1_ecmp_ipv6,data.dut2_ecmp_ipv6):
...
def test_L3Scl_ECMP_009(L3Scl_ECMP_fixture_009):
    ...
    ############################################################################################
    for _,vlan,ip1,ip2 in zip(range(0,data.max_ecmp_static),data.dut1_vlan_scl,data.dut1_ecmp_ip,data.dut2_ecmp_ip):
    ...
    tg1.tg_arp_control(handle=src1, arp_target='all')
    ...
    tg2.tg_arp_control(handle=src2, arp_target='all')
    ...
    if all_triggers_flag:
            ...
            st.wait(3)
            ports = port.get_interfaces_all(vars.D1)
            if not ports:
...
def test_L3Scl_ECMP_010(L3Scl_ECMP_fixture_010):
    ...
    ############################################################################################
    for _,vlan,ip1,ip2 in zip(range(0,data.max_ecmp_static),data.dut1_vlan_scl,data.dut1_ecmp_ipv6,data.dut2_ecmp_ipv6):
...
def create_l3_host(tg, tg_ph, host_count,src_ip,vlan,dest_ip,mac,dst_mac=None,cfg='yes'):
    ...
    trf1 = tg.tg_traffic_config(mac_dst='ff:ff:ff:ff:ff:ff',rate_pps=str_rate,mode='create',
...
def BGP_triggers(family = 'ipv4',scl_num=25000,ecmp=0):
    ...
    if family == 'ipv4':
        ipv4_scale = scl_num
        wait_time = int(ipv4_scale/(10000*perf_time))
        t = wait_time/4
        ############################################################################################
        hdrMsg("\n########## Clear bgp neighbor ############\n")
        ############################################################################################
        ...
        (tg1, tg2, _, _, _, _, _, _) = get_handles()
        st.log("BOUND_STREAM: " + str(tr1))
        st.log("BOUND_STREAM: " + str(tr2))
        res = tg1.tg_traffic_control(action='run', handle=[tr1['stream_id'], tr2['stream_id']])
        st.log("TrafControl: " + str(res))
...
def debug_cmds():
    ...
    c1 = asicapi.get_ipv4_route_count(vars.D1)
    c2 = asicapi.get_ipv4_route_count(vars.D2)
    st.log("Hardware IPv4 route count on DUT1:{} DUT2:{}".format(c1, c2))
    c1 = asicapi.get_ipv6_route_count(vars.D1)
    c2 = asicapi.get_ipv6_route_count(vars.D2)
    st.log("Hardware IPv6 route count on DUT1:{} DUT2:{}".format(c1, c2))
...
def check_ecmp():
    ...
    st.log("TrafControl: "+str(res))
    st.wait(2)
    DUT_tx_value = port.get_interface_counters(vars.D1, vars.D1D2P1,"tx_bps")
    st.log(DUT_tx_value)
    counter = 0
    vlan = 1
    ...
    for i in range(0,6):
        ...
        if int(i) != 5:
            ...
            vlan_obj.add_vlan_member(vars.D1, vlan, [vars.D1D2P1], tagging_mode=True)
            vlan_obj.add_vlan_member(vars.D2, vlan, [vars.D2D1P1], tagging_mode=True)
            port.clear_interface_counters(vars.D1)
            st.wait(2)
            DUT_tx_value = port.get_interface_counters(vars.D1, vars.D1D2P1,"tx_bps")
        else:
            tg1.tg_traffic_control(action='stop', handle=tr11['stream_id'])
    return counter
