...
from apis.routing.ip import config_ip_addr_interface,create_static_route, delete_static_route, delete_ip_interface
from apis.system.interface import clear_interface_counters, poll_for_interfaces, show_interface_counters_all, \
    get_interface_counter_value
...
from utilities.parallel import ensure_no_exception, exec_all, exec_foreach
data = SpyTestDict()
...
def initialize_variables():
    ...
    data.vlan_1 = str(utils.random_vlan_list()[0])
    ...
    data.ip_route = ["15.1.1.0", "11.1.1.0"]
    data.sub_mask = "255.255.255.0"
    data.ipv4_add = dict()
    data.ipv4_add["dut"] = [vars.D1, vars.D2]
    data.ipv4_add[vars.D1] = list()
    data.ipv4_add[vars.D2] = list()
...
def create_mirroring_policy():
    mirror.create_session(vars.D1, session_name='Mirror1', mirror_type='erspan', src_ip='11.1.1.1',
                          dst_ip='15.1.1.2', gre_type='0x88ee', dscp='50', ttl='60', queue='0')
    acl_dscp.config_policy_table(vars.D1, enable='create', policy_name='Mirror_IN_Table1',
                                 policy_type='monitoring')
    acl_dscp.config_service_policy_table(vars.D1, policy_kind='bind', policy_type='monitoring', stage='in',
                                         interface_name=vars.D1T1P1, service_policy_name='Mirror_IN_Table1')
    acl_dscp.config_service_policy_table(vars.D1, policy_kind='bind', policy_type='monitoring', stage='in',
                                         interface_name=vars.D1T1P2, service_policy_name='Mirror_IN_Table1')
    acl_dscp.config_classifier_table(vars.D1, enable='create', match_type="fields", class_name='Mirror_IN_Rule1')
    acl_dscp.config_classifier_table(vars.D1, enable='yes', class_criteria='--src-ip', match_type="fields",
                                     class_name='Mirror_IN_Rule1',
                                     criteria_value="{}/{}".format('11.1.1.2', 32))
    acl_dscp.config_flow_update_table(vars.D1,policy_name='Mirror_IN_Table1',flow='add',policy_type='monitoring',
                                      class_name='Mirror_IN_Rule1', priority_value='999',
                                      description="Mirror_AclV4")
    acl_dscp.config_flow_update_table(vars.D1,policy_name='Mirror_IN_Table1',flow='update',policy_type='monitoring',
                                      class_name='Mirror_IN_Rule1', priority_option='--mirror-session',
                                      priority_value='Mirror1')

def remove_mirroring_policy():
    acl_dscp.config_service_policy_table(vars.D1, policy_kind='unbind', stage='in', interface_name=vars.D1T1P1,
                                         service_policy_name='Mirror_IN_Table1')
    acl_dscp.config_service_policy_table(vars.D1, policy_kind='unbind', stage='in', interface_name=vars.D1T1P2,
                                         service_policy_name='Mirror_IN_Table1')
    acl_dscp.config_flow_update_table(vars.D1, policy_name='Mirror_IN_Table1',flow='del', class_name='Mirror_IN_Rule1',
                                      policy_type='monitoring')
    acl_dscp.config_policy_table(vars.D1, enable='del', policy_name='Mirror_IN_Table1')
    acl_dscp.config_classifier_table(vars.D1, enable='del', class_name='Mirror_IN_Rule1')
    mirror.delete_session(vars.D1, mirror_session='Mirror1')
...
def v4_in_acl(dut):
    ...
    acl_obj.create_acl_table(dut, name=data.acl_ipv4_table_name, stage='INGRESS', type=data.type_ipv4, description='L3 ACL',
                     ports=[])
    acl_obj.create_acl_rule(dut, table_name=data.acl_ipv4_table_name, acl_type=data.type_ipv4, rule_name='rule_1', priority=data.flow_priority[2],
                    packet_action='FORWARD', SRC_IP="{}/{}".format('1.1.1.2', '32'), DST_IP="{}/{}".format('1.1.1.3', '32'))


def v4_eg_acl(dut):
    acl_obj.create_acl_table(dut, name=data.acl_ipv4_table_name, stage='EGRESS', type=data.type_ipv4, description='L3 ACL',ports=[])
    acl_obj.create_acl_rule(dut, table_name=data.acl_ipv4_table_name, acl_type=data.type_ipv4, rule_name='rule_1', priority=data.flow_priority[2],
...
def mac_in_acl(dut):
    acl_obj.create_acl_table(dut, name=data.acl_mac_table_name, stage='INGRESS', type=data.type_mac, description='L2 ACL',
                     ports=[])
    acl_obj.create_acl_rule(dut, table_name=data.acl_mac_table_name, rule_name='rule_1', acl_type=data.type_mac,
...
def mac_eg_acl(dut):
    acl_obj.create_acl_table(dut, name=data.acl_mac_table_name, stage='EGRESS', type=data.type_mac, description='L2 ACL',
                     ports=[])
...
def v6_in_acl(dut):
    acl_obj.create_acl_table(dut, name=data.acl_ipv6_table_name, stage='INGRESS', type=data.type_ipv6,
                     description='L3V6 ACL', ports=[])
    acl_obj.create_acl_rule(dut, table_name=data.acl_ipv6_table_name, acl_type=data.type_ipv6, rule_name='rule_1', priority=data.flow_priority[2],
                    packet_action='FORWARD', SRC_IPV6="{}/{}".format('2001::2', '128'),
                    DST_IPV6="{}/{}".format('2001::3', '128'))


def v6_eg_acl(dut):
    acl_obj.create_acl_table(dut, name=data.acl_ipv6_table_name, stage='EGRESS', type=data.type_ipv6,
                     description='L3V6 ACL', ports=[])

    acl_obj.create_acl_rule(dut, table_name=data.acl_ipv6_table_name, rule_name='rule_1', acl_type=data.type_ipv6,
...
def mirrror_acl_config():
    ...
    acl_dscp.config_classifier_table(vars.D1, enable='yes', class_criteria="--vlan", match_type="fields",
                                     criteria_value=data.vlan_1, class_name='Mirror_IN_Rule1')
    acl_dscp.config_flow_update_table(vars.D1,policy_name='Mirror_IN_Table1',flow='add',policy_type='monitoring',
                                      class_name='Mirror_IN_Rule1', priority_value='999',
                                      description='Mirror_ACLV4_CREATION')
    acl_dscp.config_flow_update_table(vars.D1,policy_name='Mirror_IN_Table1',flow='update',policy_type='monitoring',
                                      class_name='Mirror_IN_Rule1', priority_option='--mirror-session',
                                      priority_value="Mirror1")
...
def delete_acl_table(dut):
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_ipv4, acl_table_name=data.acl_ipv4_table_name)
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_ipv6, acl_table_name=data.acl_ipv6_table_name)
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_mac, acl_table_name=data.acl_mac_table_name)
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_ipv4, acl_table_name=data.acl_ipv4_table_name)
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_ipv6, acl_table_name=data.cl_ipv6_table_name)
    acl_obj.clear_acl_config(vars.D1, acl_type=data.acl_type_mac, acl_table_name=data.acl_mac_table_name)
...
def acl_ratelimit_func_hooks(request):
    ...
    elif st.get_func_name(request) == "test_ft_acl_l2_l3_mirroring":
        vlan_config_delete()
        portchannel_delete()
    ...
    if st.get_func_name(request) in "test_ft_acl_ipv4_in_rate_limit_intf_vlan":
        ...
        #policy_classifier_flow_create(vars.D1)
    elif st.get_func_name(request) in "test_ft_acl_l2_l3_mirroring":
        remove_mirroring_policy()
        exec_all(True, [[delete_static_route, vars.D1, data.ip_D2D1P1, data.ip_route[0] + '/' + data.subnetMask],
                        [delete_static_route, vars.D2, data.ip_D1D2P1, data.ip_route[1] + '/' + data.subnetMask]])
        exec_foreach(True, [vars.D1, vars.D2], remove_ip_add_paraller, data.ipv4_add)
        create_portchannel(vars.D1, data.port_channel)
        add_portchannel_member(vars.D1, data.port_channel, [vars.D1D2P1, vars.D1D2P2])
        create_vlan(vars.D1, data.vlan_1)
        add_vlan_member(vars.D1, data.vlan_1, [vars.D1T1P1, vars.D1T1P2, data.port_channel], tagging_mode=True)
...
def verify_traffic(dut, src_intf, dst_intf, stage):
    ...
    ingress_rx_cnt = utils.filter_and_select(counter_output, ['rx_ok'], {'iface': src_intf})[0]['rx_ok']
    ingress_rx_drp_cnt = utils.filter_and_select(counter_output, ['rx_drp'], {'iface': src_intf})[0]['rx_drp']
    egress_tx_cnt = utils.filter_and_select(counter_output, ['tx_ok'], {'iface': dst_intf})[0]['tx_ok']
    egress_tx_drp_cnt = utils.filter_and_select(counter_output, ['tx_drp'], {'iface': dst_intf})[0]['tx_drp']
...
def policy_flow_test_ipv4(dut, interface_name, policy_name, stage, src_intf, dst_intf, line_rate_traffic=False):
    ...
    acl_dscp.show(dut, interface_name=interface_name)
...
def config_ip_add_paraller(dut, params):
    if params:
        for data in params[dut]:
            config_ip_addr_interface(dut, data["port"], data["ip_addr"], data["subnet"])

def remove_ip_add_paraller(dut, params):
    if params:
        for data in params[dut]:
            delete_ip_interface(dut, data["port"], data["ip_addr"], data["subnet"])
...
@pytest.mark.regression
def test_ft_acl_l2_l3_mirroring():
    st.log("Creating IP address configuration on DUT-1")
    for port, ip_addr, in zip(data.port_list_d1, data.ip_list_d1):
        data.ipv4_add[vars.D1].append({"port": port, "ip_addr": ip_addr, "subnet": data.subnetMask})
    st.log("Creating IP address configuration on DUT-2")
    for port, ip_addr in zip(data.port_list_d2, data.ip_list_d2):
        data.ipv4_add[vars.D2].append({"port": port, "ip_addr": ip_addr, "subnet": data.subnetMask})
    [_, exceptions] = exec_foreach(True, [vars.D1, vars.D2], config_ip_add_paraller, data.ipv4_add)
    ensure_no_exception(exceptions)

    st.log("Creatting static route")
    [_, exceptions] = exec_all(True, [[create_static_route, vars.D1, data.ip_D2D1P1, data.ip_route[0] + '/' + data.subnetMask],
                                      [create_static_route, vars.D2, data.ip_D1D2P1, data.ip_route[1] + '/' + data.subnetMask]])
    ensure_no_exception(exceptions)

    create_mirroring_policy()

    if not mirror.verify_session(vars.D1, mirror_type="erspan", session_name='Mirror1'):
        st.report_fail("mirror_session_verification_failed")
    [_, exceptions] = exec_all(True, [
        [clear_interface_counters, vars.D1],
        [clear_interface_counters, vars.D2]])
    ensure_no_exception(exceptions)

    dut_mac = basic.get_ifconfig_ether(vars.D1, vars.D1T1P1)
    data.tg = tgapi.get_chassis(vars)

    st.log("Creating IPv4 routing interfaces on TG ports")
    data.tg.tg_interface_config(port_handle=data.tg_ph_1, mode='config',
                                       intf_ip_addr=data.ip_T1D1P1,
                                       gateway=data.ip_D1T1P1, netmask=data.sub_mask, count=3,
                                       arp_send_req='1')
    data.tg.tg_interface_config(port_handle=data.tg_ph_2, mode='config',
                                       intf_ip_addr=data.ip_T1D1P2,
                                       gateway=data.ip_D1T1P2, netmask=data.sub_mask, count=3,
                                       arp_send_req='1')
    data.tg.tg_interface_config(port_handle=data.tg_ph_3, mode='config',
                                       intf_ip_addr=data.ip_T1D2P1,
                                       gateway=data.ip_D2T1P1, netmask=data.sub_mask, count=3,
                                       arp_send_req='1')
    tr5 = data.tg.tg_traffic_config(port_handle=data.tg_ph_1, mode='create', transmit_mode='single_burst',
                                           length_mode='fixed', pkts_per_burst=1000,
                                           rate_pps=100, l2_encap='ethernet_ii',
                                           mac_src='00:0a:01:00:00:01',
                                           mac_dst=dut_mac, l3_protocol="ipv4", ip_src_addr=data.ip_T1D1P1,
                                           ip_dst_addr=data.ip_D1T1P2, vlan_id=data.vlan_1, vlan="enable")

    data.tg.tg_traffic_control(action='run', handle=tr5['stream_id'])
    st.wait(5)
    data.tg.tg_traffic_control(action='stop', handle=tr5['stream_id'])
    st.wait(5)

    receive_counters_from_tg = get_interface_counter_value(vars.D1, vars.D1T1P1, "rx_ok")
    st.log("receive_counters_from_tg = {} " . format(receive_counters_from_tg))
    receive_count_int_value = int(receive_counters_from_tg[vars.D1T1P1]['rx_ok'])
    st.log("receive_count_int_value = {} " .format(receive_count_int_value))

    tx_counters_from_mirrored_port = get_interface_counter_value(vars.D2, vars.D2T1P1, "tx_ok")
    st.log("tx_counters_from_mirrored_port = {} " . format(tx_counters_from_mirrored_port))
    tx_counters_from_mirrored_port_int_value = int(tx_counters_from_mirrored_port[vars.D2T1P1]['tx_ok'])
...
def test_ft_acl_rate_limit_shut_noshut_interface():
    ...
    if data.flag == 0:
        acl_obj.clear_acl_config(vars.D1, data.acl_ipv4_table_name)
        bind_policy_and_verify_status(vars.D1, vars.D1T1P1, data.stage[0], 'single_rate', 'del')
        st.report_fail("Flow_operation_failed")
...
def test_ft_acl_rate_limit_policy_scalability():
    ...
    for element in range(1, data.max_sections + 2):
        ...
        acl_obj.create_acl_table(vars.D1, name=acl_name, stage='INGRESS', type=data.type_ipv4, description='L3 ACL', ports=[])
...
def test_ft_verify_acl_rate_limit_priority_over_intf():
    ...
    acl_obj.create_acl_table(vars.D1, name=data.acl_ipv4_table_name_1, stage='INGRESS', type=data.type_ipv4,
                     description='L3 ACL',
                     ports=[])
    acl_obj.create_acl_table(vars.D1, name=data.acl_ipv4_table_name_2, stage='INGRESS', type=data.type_ipv4,
                     description='L3 ACL',
                     ports=[])

    acl_obj.create_acl_rule(vars.D1, table_name=data.acl_ipv4_table_name_1, rule_name='rule_1', acl_type=data.type_ipv4,
                    priority=data.flow_priority[2], packet_action='FORWARD', SRC_IP="{}/{}".format('1.1.1.2', '32'),
                    DST_IP="{}/{}".format('1.1.1.3', '32'))
    acl_obj.create_acl_rule(vars.D1, table_name=data.acl_ipv4_table_name_2, rule_name='rule_1', acl_type=data.type_ipv4,
                    priority=data.flow_priority[2], packet_action='FORWARD', SRC_IP="{}/{}".format('1.1.1.2', '32'),
                    DST_IP="{}/{}".format('1.1.1.3', '32'))
...
def test_ft_acl_rate_limit_section_priority():
    ...
    acl_obj.create_acl_table(vars.D1, name=data.acl_ipv4_table_name_1, stage='INGRESS', type=data.type_ipv4,
                                      description='L3 ACL',
                                      ports=[])
    acl_obj.create_acl_rule(vars.D1, table_name=data.acl_ipv4_table_name_1, rule_name=data.rule_name2,acl_type=data.type_ipv4,
